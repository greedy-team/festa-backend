...
public interface ArtistRepository extends JpaRepository<Artist, Long> {
    ...
    @Query(value = """
            ...
            WHERE (:needsReview IS NULL OR a.needsReview = :needsReview)
              ...
              AND (:q IS NULL
                   OR LOWER(REPLACE(a.name, ' ', '')) LIKE LOWER(CONCAT('%', REPLACE(CAST(:q AS String), ' ', ''), '%')) ESCAPE '\\'
                   OR EXISTS (SELECT 1 FROM ArtistAlias al
                              WHERE al.artist = a
                                AND LOWER(REPLACE(al.name, ' ', '')) LIKE LOWER(CONCAT('%', REPLACE(CAST(:q AS String), ' ', ''), '%')) ESCAPE '\\'))
            GROUP BY a
            """,
            countQuery = """
            SELECT COUNT(a)
    ...
    @Query(value = """
            SELECT a AS artist, COUNT(DISTINCT f) AS appearanceCount
            FROM Artist a
            LEFT JOIN Lineup l ON l.artist = a
            LEFT JOIN Festival f ON f = l.festival
                 AND f.publishedAt IS NOT NULL
    ...
    @Query("""
            ...
            WHERE l.artist.id IN :artistIds
              AND f.publishedAt IS NOT NULL
              AND f.endDate < :today
            GROUP BY l.artist.id, f.id, f.name, h.shortName, f.startDate
            ORDER BY l.artist.id ASC, f.startDate DESC, f.id DESC
            """)
    ...
    @Query("""
            ...
            LEFT JOIN Festival f ON f = l.festival
                 AND f.publishedAt IS NOT NULL
                 AND f.endDate < :today
            WHERE LOWER(REPLACE(a.name, ' ', '')) LIKE LOWER(CONCAT('%', REPLACE(CAST(:q AS String), ' ', ''), '%')) ESCAPE '\\'
               OR EXISTS (SELECT 1 FROM ArtistAlias al
                          WHERE al.artist = a
                            AND LOWER(REPLACE(al.name, ' ', '')) LIKE LOWER(CONCAT('%', REPLACE(CAST(:q AS String), ' ', ''), '%')) ESCAPE '\\')
            GROUP BY a

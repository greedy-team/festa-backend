...
--
-- short_name은 통용 약칭이다. 표기 규칙은 DEC-0098로 확정했다 — `X대학교`→`X대`,
-- `X여자대학교`→`X여대`. 규칙 산출물이 통용 약칭과 다른 둘만 예외로 뒀다:
-- 한국외국어대학교→한국외대, 서울과학기술대학교→서울과기대.
-- `연대`·`고대` 같은 1음절 약칭은 쓰지 않는다 — 동대(동국대/동덕여대)처럼 겹친다.
-- 표시 전용 값이며 매칭 키는 name이다. 이 컬럼은 조회·임포트 어디에도 쓰이지 않는다.
--

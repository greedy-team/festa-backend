...
운영 배포는 새 버전을 띄운 뒤 옛 버전을 멈춥니다. 그 사이 **두 버전이 같은 DB를 함께 씁니다.**
되돌리기도 옛 버전을 그대로 되살리는 방식이라, 마이그레이션은 **직전 버전 코드가 돌 수 있는 모양**이어야 합니다.

**한 릴리스로 가능**

- 테이블 추가, nullable 컬럼 추가, 기본값 있는 컬럼 추가, 인덱스 추가, 시드 데이터 추가

**두 릴리스로 나눕니다**

| 하려는 것 | ① 이번 릴리스 | ② 다음 릴리스 |
| --- | --- | --- |
| 컬럼·테이블 삭제 | 코드에서 사용 중단 | 삭제 |
| 이름 변경 | 새 컬럼 추가·값 복사, 코드 전환 | 옛 컬럼 삭제 |
| 타입 변경 | 새 컬럼으로 옮김 | 옛 컬럼 삭제 |
| NOT NULL·UNIQUE 추가 | 옛 코드가 값을 넣도록 먼저 고침 | 제약 추가 |

**되돌리기가 되는 근거 둘** — 이 설정을 바꾸면 되돌리기가 깨집니다.

- Flyway 기본값 `ignoreMigrationPatterns=*:future` — DB에 더 새로운 마이그레이션이 적용돼 있어도 옛 버전이 뜹니다
- Hibernate `ddl-auto: validate` — 엔티티가 모르는 컬럼·테이블이 더 있어도 통과합니다

마이그레이션 CI 검사는 두지 않습니다. 위반이 한 번이라도 생기면 그때 붙입니다.

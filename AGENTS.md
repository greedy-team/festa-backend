...
**선택값과 필수값을 가릅니다.** 앱이 그 값 없이도 떠야 하면 compose에서 `${VAR:-}`로 두고
`application.yml`에는 `${VAR:}` 기본값을 줍니다. `${VAR:?}`는 **없으면 앱이 못 뜨는 값에만**
씁니다 — compose는 모든 환경이 공유하므로 `:?`는 그 값을 아직 등록하지 않은 환경의 첫 배포를
막고, 같은 해석 오류가 롤백 분기에서도 나서 "롤백까지 실패"로 읽힙니다 (이슈 #47).

**형식이 중요한 비밀은 CD의 기존 스텝 안에서 검사합니다.** 새 스텝을 늘리지 않습니다.
디코딩 결과만 보지 말고 **원문을 먼저** 보세요 — `base64 -d`는 줄바꿈이 섞인 값도 통과시킵니다.
검사 실패 메시지에 값을 출력하지 않습니다.

**스위치는 리터럴로 둡니다.** `..._ENABLED: "true"` / `"false"`. 빈 문자열은 Boot에서
기본값 `true`로 바인딩되므로, 끌 때 값을 비우거나 URL 줄을 지우지 않습니다.
예: `MANAGEMENT_OTLP_METRICS_EXPORT_ENABLED`(메트릭 전송).
